from flask import render_template, send_file, current_app, request, flash, redirect, url_for
...
def generar_certificados():
    ...
    for _, row in participants.iterrows():
        ...
        pdf.add_page()
        template_path = os.path.join(os.path.dirname(__file__), "input", "certificate_template.jpg")
...
def enviar_certificado(user_id):
    ...
    with db.engine.connect() as conn:
        query = text("SELECT nombre, apellido, email FROM usuarios WHERE id_usuario = :id")
    ...
    return render_template("send_certificate.html", student=student, apellido=apellido, email=email)
...
def enviar_certificados_todos():
    if request.method == "POST":
        ...
        with db.engine.connect() as conn:
            query = text("SELECT id_usuario, nombre, apellido, email FROM usuarios WHERE id_usuario=4 OR id_usuario=6")
