...
def crear_curso(nombre, descripcion, modalidad, id_version, id_ponente):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO cursos (nombre, descripcion, modalidad, id_version, id_ponente) VALUES (%s,%s,%s,%s,%s)", (nombre, descripcion, modalidad, id_version, id_ponente,))
    conn.commit()
    conn.close()
...
def actualizar_curso(id_curso, nombre, descripcion, modalidad, id_version, id_ponente):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE cursos SET nombre = %s, descripcion = %s, modalidad = %s, id_version = %s, id_ponente = %s WHERE id_curso = %s",
